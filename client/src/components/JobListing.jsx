...
const JobListing = () => {
    ...
    const [filteredJobs, setFilteredJobs] = useState(jobs); // getting this jobs from context
  ...
  useEffect(()=>{
      ...
      const matchedLocations = job => selectedLocations.length === 0 || selectedLocations.includes(job.location)
      ...
      const newFilteredJobs = jobs.slice().reverse().filter(
         job => matchesCategory(job) && matchedLocations(job) && matchesTitle(job) && matchesSearchLocation(job)
      )
                ...
                </a>
                {
                    Array.from({length : Math.ceil(filteredJobs.length/6)}).map((_,ind)=>(
                        <a key={ind} href="#job-list">
                            <button onClick={()=> setCurrentPage(ind + 1)} className={`pl-3 w-10 h-10 flex items-center border border-gray-300 rounded ${currentPage === ind +1 ? 'bg-blue-100 text-blue-500': 'text-gray-500 '}`}>{ind+1}</button>
                        </a>
                    ))
                }
                <a href="#job-list">
